...
before the mutation landed describes pre-mutation state, so it is still returned
to the caller that asked for it, but is not written to the cache — the next read

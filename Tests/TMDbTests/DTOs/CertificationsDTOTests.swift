...
class CertificationsDTOTests: XCTestCase {

    func testDecodeReturnsCertifications() throws {
        let data = json.data(using: .utf8)!
        let result = try JSONDecoder.theMovieDatabase.decode(CertificationsDTO.self, from: data)
    ...
    private let certifications = CertificationsDTO(
        certifications: [
            "US": [
                CertificationDTO(
                    code: "G",

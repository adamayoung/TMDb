//
//  FindServiceConvenienceTests.swift
//  TMDb
//
...
///
/// Pins the zero-defaulted-argument conveniences on ``FindService``.
///
...
///
@Suite(.tags(.testingSupport, .mocks, .find))
struct FindServiceConvenienceTests {

    var service: MockFindService

    init() {
        self.service = MockFindService()
    }

    @Test("find(externalID:externalSource:) forwards a nil language to the requirement")
    func findForwardsNilLanguage() async throws {
        _ = try await service.find(externalID: "tt0137523", externalSource: .imdbID)

        #expect(service.findCalls.count == 1)
        let call = try #require(service.findCalls.first)
        #expect(call.language == nil)
        #expect(call.externalID == "tt0137523")
        #expect(call.externalSource == .imdbID)
    }
